...
import com.csy.afv.utils.LogUtil.logW
...
class DownloadAdapter(context: Context, list: ArrayList<VideoBean>) : RecyclerView.Adapter<DownloadAdapter.DownloadViewHolder>() {
    ...
    override fun onBindViewHolder(holder: DownloadViewHolder, position: Int) {
        ...
        isDownload = SharedPreUtils.getInstance(context!!, "download_state").getBoolean(list?.get(position)?.playUrl!!)
        ...
        holder.iv_download_state.setOnClickListener {
            if (isDownload) {
                isDownload = false
                SharedPreUtils.getInstance(context!!, "download_state").put(list?.get(position)?.playUrl!!, false)
                holder.iv_download_state.setImageResource(R.drawable.icon_download_start)
                RxDownload.getInstance(context).pauseServiceDownload(list?.get(position)?.playUrl).subscribe()
            } else {
                isDownload = true
                SharedPreUtils.getInstance(context!!, "download_state").put(list?.get(position)?.playUrl!!, true)
                holder.iv_download_state.setImageResource(R.drawable.icon_download_stop)
        ...
        disposable = RxDownload.getInstance(context).receiveDownloadStatus(playUrl)
                .subscribe { event ->
                    ...
                    if (percent == 100L) {
                        ...
                        isDownload = false
                        SharedPreUtils.getInstance(context!!, "download_state").put(playUrl.toString(), false)
                    } else {
