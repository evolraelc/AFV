 ...
 * @Author:CSY
 * @Date:
 * @Description: 通过对象与字符串间的转换，设置具体的方法来获取具体的键值对
 */
class SharedPreUtils private constructor(context: Context, spName: String) {
    ...
    companion object {
        ...
        private val sSPMap = HashMap<String,SharedPreUtils>()
         ...
         */
        fun getInstance(context: Context,spName: String): SharedPreUtils {
            var spName = spName
            if (isSpace(spName)) spName = "spUtils"
            var sharedPre: SharedPreUtils? = sSPMap[spName]
            if (sharedPre == null) {
                sharedPre = SharedPreUtils(context,spName)
                sSPMap.put(spName, sharedPre)
            }
            return sharedPre
        }
